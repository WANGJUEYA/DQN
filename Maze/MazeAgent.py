...
class MazeAgent(object):
    ...
    def save_model(self, model_name="maze_dqn.pth"):
        """保存模型"""
    ...
    def load_model(self, model_name="maze_dqn.pth"):
        """加载模型"""
        model_path = os.path.join(MODEL_SAVE_DIR, model_name)
...
def inference_demo(model_name="maze_dqn.pth", episodes=5):
    """推理演示函数"""
    print("=" * 50)
    print("开始推理演示...")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "inference":
        # 推理模式
        model_name = sys.argv[2] if len(sys.argv) > 2 else "maze_dqn.pth"
        ...
        for i in range(EPOCH):  # episode循环
            ...
            # 每50个episode保存一次模型
            if (i + 1) % 50 == 0:
                dqn.save_model(f"maze_dqn_episode_{i+1}.pth")
        
        # 训练完成后保存最终模型
        dqn.save_model("maze_dqn_final.pth")
        env.close()
        print("\n训练完成！模型已保存。")
        print("使用方法:")
        print("python MazeAgent.py inference                    # 使用默认模型进行推理")
        print("python MazeAgent.py inference model_name.pth     # 使用指定模型进行推理")
        print("python MazeAgent.py inference model_name.pth 10  # 使用指定模型进行10次推理")
