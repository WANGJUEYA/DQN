...
class Net(nn.Module):
    ...
    def __init__(self, agent: BaseAgent):  # 定义Net的一系列属性
        ...
        self.fc1 = nn.Linear(agent.num_states, 20)  # 设置第一个全连接层(输入层到隐藏层): 状态数个神经元到20个神经元
        self.fc1.weight.data.normal_(0, 0.1)  # 权重初始化 (均值为0，方差为0.1的正态分布)
        self.fc2 = nn.Linear(20, agent.num_actions)  # 设置第二个全连接层(隐藏层到输出层): 20个神经元到动作数个神经元
...
class CartPoleAgent(BaseAgent):
    ...
    def _init_env_and_agent(self):
        ...
        self.num_actions = self.env.action_space.n  # 杆子动作个数 (2个)
        self.num_states = self.env.observation_space.shape[0]  # 杆子状态个数 (4个)
